...
STATIC mp_obj_t usb_config_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *args) {
    ...
    mp_map_t kws;
    mp_map_init_fixed_table(&kws, n_kw, args + n_args);

    usb_config_obj_t *self = m_new_obj(usb_config_obj_t);
    memset(self, 0, sizeof(usb_config_obj_t));
    self->base.type = type;
    tusb_config_init(&self->tusb_config);

    self->tusb_config.host = false;

    mp_map_elem_t *elem;
    elem = mp_map_lookup(&kws, MP_OBJ_NEW_QSTR(MP_QSTR_cdc_itf), MP_MAP_LOOKUP);
    self->tusb_config.cdc_itf = elem ? mp_obj_get_int(elem->value) : -1;

    elem = mp_map_lookup(&kws, MP_OBJ_NEW_QSTR(MP_QSTR_disconnect), MP_MAP_LOOKUP);
    self->tusb_config.disconnect = elem ? mp_obj_is_true(elem->value) : false;
...
STATIC mp_obj_t usb_config_device(size_t n_args, const mp_obj_t *args, mp_map_t *kws) {
    ...
    tusb_desc_device_t *desc = &self->device;
...
STATIC mp_obj_t usb_config_configuration(size_t n_args, const mp_obj_t *args, mp_map_t *kws) {
    ...
    size_t idx = self->device.bNumConfigurations;
    ...
    self->device.bNumConfigurations++;
    self->cfg_idx = idx;
    self->ep_idx = 1;
...
STATIC tusb_desc_configuration_t **usb_config_cfg_get(usb_config_obj_t *self) {
    ...
    assert(self->cfg_idx < self->device.bNumConfigurations);
...
STATIC mp_obj_t usb_config_cdc(size_t n_args, const mp_obj_t *args, mp_map_t *kws) {
    usb_config_obj_t *self = MP_OBJ_TO_PTR(args[0]);
    if (!self->tusb_config.device || (self->cfg_idx >= self->device.bNumConfigurations)) {
        mp_raise_ValueError(NULL);
    }
...
STATIC void usb_config_print_tusb_config(const mp_print_t *print, const tusb_config_t *tusb_config) {
    ...
    for (size_t i = 0; i < tusb_config->device->bNumConfigurations; i++) {
        tusb_desc_configuration_t *cfg = tusb_config->configs[i];
        mp_printf(print, "    { ");
    ...
    for (size_t i = 0; i < TUSB_CONFIG_MAX_STRS; i++) {
        tusb_desc_string_t *str = tusb_config->strings[i];
        if (!str) {
            break;
        }
        mp_printf(print, "    { ");
...
STATIC mp_obj_t usb_config_exists(mp_obj_t self_in) {
    return mp_obj_new_bool(tusb_config_get() != NULL);
}
STATIC MP_DEFINE_CONST_FUN_OBJ_1(usb_config_exists_obj, usb_config_exists);

STATIC mp_obj_t usb_config_delete(mp_obj_t self_in) {
    tusb_config_delete();
    return mp_const_none;
}
STATIC MP_DEFINE_CONST_FUN_OBJ_1(usb_config_delete_obj, usb_config_delete);

STATIC mp_obj_t usb_config_save(mp_obj_t self_in) {
    usb_config_obj_t *self = MP_OBJ_TO_PTR(self_in);
    size_t len = tusb_config_save(&self->tusb_config);
    return MP_OBJ_NEW_SMALL_INT(len);
}
STATIC MP_DEFINE_CONST_FUN_OBJ_1(usb_config_save_obj, usb_config_save);

STATIC mp_obj_t usb_config_load(mp_obj_t self_in) {
    usb_config_obj_t *self = MP_OBJ_TO_PTR(self_in);
    const tusb_config_t *tusb_config = tusb_config_get();
    if (!tusb_config) {
        mp_raise_ValueError(NULL);
    }
    self->tusb_config = *tusb_config;
    return mp_const_none;
}
STATIC MP_DEFINE_CONST_FUN_OBJ_1(usb_config_load_obj, usb_config_load);
